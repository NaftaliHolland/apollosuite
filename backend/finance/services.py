...
def recalculate_student_discounts(student, academic_year):
    ...
    fee_item_specific_discounts = student_discounts.exclude(
        discount__fee_item__isnull=True
    ).all()
    ...
    for student_discount in fee_item_specific_discounts:
        ...
        student_fee_item_assignment = StudentFeeAssignment.objects.get(
            student=student, grade_fee_item__fee_item=fee_item
        )
    ...
    for student_discount in general_discounts:
        ...
        total_gross = student_fee_assignments.aggregate(Sum("gross_amount"))[
            "gross_amount__sum"
        ]
        ...
        for student_fee_assignment in student_fee_assignments:
            if discount_value_type == "percentage":
                discount_amount = (
                    discount_value / 100
                ) * student_fee_assignment.gross_amount

            elif discount_value_type == "fixed":
                discount_amount = (
                    student_fee_assignment.gross_amount / total_gross
                ) * discount_value
...
def record_payment(
    ...
    payments = Payment.objects.bulk_create(payment_objects)

    PaymentItem.objects.bulk_create(payment_item_objects)

    return payments


def get_student_fee_summary(
    student: StudentProfile, academic_year: AcademicYear, term: Term | None = None
):
    # Get student details
    ...
    total_fees_due = sum(student_fee_assignment.gross_amount for student_fee_assignment in student_fee_assignments)
    
    previously_paid = sum(student_fee_assignment.amount_paid for student_fee_assignment in student_fee_assignments)

    discounts = sum(student_fee_assignment.discount_amount for student_fee_assignment in student_fee_assignments)
    ...
    fee_summary["summary"] = {
        ...
        "discounts_applied": discounts, # Sum of all discounts
        "outstanding_balance": balance, # Still needs to pay
